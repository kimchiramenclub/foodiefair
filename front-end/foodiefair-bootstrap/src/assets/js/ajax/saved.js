...
async function fetchSavedList(userId) {
    const response = await fetch(`https://www.foodiefair.shop/mypage/${userId}/saved-examples`);
...
async function registerSavedProduct(productId, userId) {
    ...
    const response = await fetch(`https://www.foodiefair.shop/products/${productId}/saved`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
