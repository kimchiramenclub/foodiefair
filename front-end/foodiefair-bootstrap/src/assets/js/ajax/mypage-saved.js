...
async function fetchSavedList(userId, page = 1, size = 16) {
    const response = await fetch(`https://115.85.183.196:8081/mypage/${userId}/saved-products?page=${page}&size=${size}`);
    return await response.json();
}

async function removeSavedProduct(productId, userId) {
    const response = await fetch(`https://115.85.183.196:8081/products/${productId}/saved/mypage?userId=${userId}`, {
        method: 'DELETE',
        headers: {
...
async function registerSavedProduct(productId, userId) {
    ...
    const response = await fetch(`https://115.85.183.196:8081/products/${productId}/saved/mypage`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
async function fetchProductCount(userId) {
    const response = await fetch(`https://115.85.183.196:8081/mypage/${userId}/saved-products/count`);
