...
async function productReviewsRead(e) { // 상품 리뷰들 목록 가져오기
    ...
    const response = await fetch('https://115.85.183.196/products/review/reviewRead?'+$.param(queryString)); // 서버에 데이터 요청 후 응답 기다림. 반환 데이터는 Promise 객체
    ...
    let likeReviewList = [];
    if (userInfo) {
        const likeReviewResponse = await fetch('https://115.85.183.196/products/review/likeReview/'+userInfo.userId);
...
async function productReviewLike(e) { // 상품 리뷰 좋아요 토글 버튼
    ...
    if ($(this).hasClass('active')) {  // 토글 활성화시 데이터 저장
        ...
        const response = await fetch('https://115.85.183.196/products/review/likeReview', { // 좋아요 총 개수 Promise형태로 반환
            method:'POST',
            headers: {
        ...
        const response = await fetch('https://115.85.183.196/products/review/likeReview/'+sendData.reviewId+'/'+sendData.userId, { // 좋아요 총 개수 Promise형태로 반환
            method:'DELETE'
        });
        const data = await response.json(); // Promise형태를 json으로, Integer 값이라 바로 사용 가능한듯
        await $(this).find('.badge').text(data); // 하위 자식중에 badge를 찾아서 좋아요 개수 업데이트
...
async function productReviewRemove (e) {
    ...
    const response = await fetch('https://115.85.183.196/products/review/reviewDelete/'+reviewId, {
        method:'DELETE'
    });
...
async function productReviewModify(e) {
    ...
    const response = await fetch('https://115.85.183.196/products/review/reviewReadOne/' + reviewId, {
        method: 'GET'
    });
...
async function reviewUpDate (e) {
    ...
    await fetch('https://115.85.183.196/products/review/reviewModify', {
        method: 'PATCH',
        body: formData
    });
    $('#review-section').empty();
